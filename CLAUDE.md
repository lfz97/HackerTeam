...
## Architecture
- Multi-agent AI pentesting platform: Captain maintains an adaptive `todo_write` plan and serially dispatches one specialized Agent at a time according to the current evidence need. PTES is a reference checklist, not a fixed Agent sequence; Agents may be skipped, repeated, or revisited. Pure reasoning/file-analysis tasks may be handled directly by Captain; simple CTFs and one-off command/script/network tasks route to Exploit's general execution mode.
- Current role contracts: Recon owns asset/service/version/path/WAF discovery; Scanner owns automated vulnerability-candidate scanning against concrete targets only; Exploit owns concrete verification, controlled initial exploitation, and bounded CTF/general execution fallback; PostExploit owns exactly one Captain-authorized objective from existing access; Reproducer owns offline script generation from reviewed structured vulnerability evidence.
...
- `LocalExec.run` blocks up to 20s — it returns status/exit code/output inline if the command finishes within that window, otherwise switches it to background execution and returns the job id; agents then use `status` / `output` / `intervene` / `kill`. The old `submit_command` + mandatory-poll workflow is gone
- `localexec.Manager` is per-agent, not a global singleton — `LocalExec()` creates a new Manager for each `LocalExecToolSet` instance; global `cache.go` removed。**实例启动时建一次（`initBuiltinToolsets()`，每个执行角色一个），跨轮复用不重建**——上一轮 run 提交的长任务下一轮仍可 `status`/`output` 续查；`Close()` 会先 kill 所有未结束命令再清空注册表
...
- **框架工具名前缀** — toolset 工具运行时名是 `{toolsetName}_{toolName}`（如 localexec `WithName("run")` + toolset 名 `"LocalExec"` → LLM 看到 `LocalExec_run`）。mapper 须用带前缀名；localexec 用 `localExecToolSetName+"_"+toolName` 构造。新增 toolset 工具的字段映射记得用前缀名。
...
- **框架内置工具（todo_write，Captain 专属）** — 来自框架（`tool/todo`），封装在 `tools/functions/todo.go`：任务清单写进 session state（`temp:todos[:branch]`，按 invocation branch 分键隔离，子 Agent 不会覆盖队长计划）；**只挂给 Captain**——在 `initCaptain`（`members.go`）单独 `append`，刻意不放进 `initBuiltinTools` 的共享 `builtinTools`（否则 5 个子 Agent 全会拿到）；使用说明由 `{{TODO_PROMPT}}` 占位符注入 `prompts/agents/captain.md`，内容取 `todo.DefaultToolPrompt`（`assemblePrompt` 替换，随框架版本走，不硬编码）。渲染字段在 `jsonmapper.go` 用常量 `todoWriteToolName` 注册，只显示 `message`（避免整份清单 JSON 刷屏）。**注意**：框架还有个 `await_user_reply`（`tool/awaitreply`）追问路由工具，**本项目未启用**——路由目标是"调用 await 的那个 agent"，但当前子 Agent 走 agenttool 挂载（非 `WithSubAgents` 注册），runner 的 `FindSubAgent` 解析不到子 Agent 路径，且只挂 Captain（根）时路由本身也是 no-op；将来把子 Agent 改为 `WithSubAgents` 注册并给它们开启该工具，"子 Agent 追问后用户消息直达该子 Agent"才有实际价值。
...
- **Agent status bar (BeforeModel callback)** — `setBeforeModelStatusCallback()`（`service/engine/members.go`）在每次 LLM 调用时向请求**末尾** append 一条 system 状态栏（TIMENOW/CWD/MEMORY）。三条不可违反的设计约束：① 必须 append 到**尾部**而非 prepend——状态栏内容每次调用都变，放头部会破坏服务端自动前缀缓存（实测：尾部 95-99% 命中 vs 头部 0）；② 必须配套 `openai.WithOptimizeForCache(false)`（`service/engine/models/openai.go`）——否则框架的 `optimizeMessagesForCache` 会把尾部 system 重排到头部，缓存收益失效；③ `{{DATE}}`/`{{CWD}}` 占位符已从 `env.md`/`init.go` 移除（与状态栏重叠）。状态栏不进 session（仅存在于当次请求副本），不污染摘要/压缩。详见 `docs/agent-time-awareness-callbacks.md`。**Todo 注入**：同一 callback 还会追加 `functionTools.TodoStatusBar(ctx)`——经 `agent.InvocationFromContext(ctx)` 取回 invocation（BeforeModel 的 ctx 由框架 `llmflow.go` `withInvocationContextIfMissing` 兜底携带），用框架公开的 `todo.GetTodos(inv.Session, inv.Branch)` 读清单（与 todo_write 同 key `temp:todos[:branch]`），渲染成单行紧凑摘要（`[TODO] ◐ 进行中 | ☐ 待办... (N done)`，pending 最多 8 条、单条 80 rune 截断）。无清单返回空串不追加。清单变化只影响尾部消息不破坏前缀缓存；todo_write 写入后同轮下一跳请求即生效，上下文压缩只裁历史不动 session state，清单不丢。
...
- **错误自动重试（errorStreak 语义 = "配置层健康检查"，非"有界重试预算"）** — `engineCore.go` 常量 `errorMaxTimes=3`/`errorSleepGap=3s` + `Engine.errorStreak`。归零时机：收到任何带 Choices 的 Response 事件（第一个 token 即归，含流式部分块——能吐 token = key/端点/鉴权/本地网络全通，故障只剩传输层抖动，随机性强、重试期望为正，自动重试到成功为止）、/new、ESC(Int)、`AgentStart` 的 else 兜底；**手动提交输入不归零**。耗尽只可能由零输出失败（401/连接拒绝/上下文超限 400）触发，"连续 N 次失败"的实际含义是"检查配置"；中途死亡类失败（流式出字后断）永不耗尽——确定性中途失败（超时、内容过滤）会无限自动重试是刻意接受的代价，配套动作是把主模型 `httptimeout` 调大。耗尽后 Code 保持 Error、streak 不归零、整个复用 `*EndTurn_p`。`agentRunIteratively` 输入循环是"Error 优先 + else 兜底"结构，不要改回列举式（新增 turnCode 会落进安全兜底）。
