...
**首次运行**会在可执行文件同目录自动生成配置目录，随后退出并提示填入 API Key。

同时，系统会为 4 个子 Agent 各自创建独立的 Skill 目录，并预置一份**渗透测试工具 Skill 模板**（`SKILL.md.template`），内含 nmap、fscan、dirsearch、sqlmap、nuclei 等常用工具的路径与分类信息：

```
.HackerTeam/
├── HackerTeam.yaml          ← 主配置文件（填入 API Key 即可运行）
├── HackerTeam.log
├── ReconSkills/             ← Recon 子 Agent 的 Skill 目录
│   └── pentest-tools/
│       └── SKILL.md.template    ← 预置工具 Skill 模板（需 rename 激活）
├── VulnAnalyzeSkills/       ← VulnAnalyst 子 Agent 的 Skill 目录
│   └── pentest-tools/
│       └── SKILL.md.template
├── ExploitSkills/           ← Exploit 子 Agent 的 Skill 目录
│   └── pentest-tools/
│       └── SKILL.md.template
└── PostExploitSkills/       ← PostExploit 子 Agent 的 Skill 目录
    └── pentest-tools/
        └── SKILL.md.template
```

> 模板文件以 `.template` 为后缀，不会被 Agent 自动加载。使用时需重命名为 `SKILL.md`。**强烈建议按需定制：** 将每个 Agent 所需使用的工具信息（名称、路径、参数、输出格式等）配置到对应的 Skill 目录中。模板中的默认路径仅供参考，请替换为你的工具实际安装位置。Agent 只会「知道」你写进 SKILL.md 的工具。

修改配置后再次运行即进入 TUI 交互界面。
...
- 支持自由增删 Skill 文件，执行 `/flush` 命令**无需重启**即可热更新

### 预置模板

首次运行时，每个 Agent 的 Skill 目录下会自动生成 `pentest-tools/SKILL.md.template`（`.template` 后缀防止被框架直接加载），预置了常用渗透工具的参考条目：
...
使用时将 `SKILL.md.template` 重命名为 `SKILL.md`，Agent 启动时即自动注入到系统提示词中。执行 `/flush` 无需重启即可热更新。

> **强烈建议按需定制：** 根据每个 Agent 的实际职责，为其 Skill 目录配置对应的工具。
...
>
> 模板中的路径为占位示例，请替换为工具的实际安装路径。Agent 只会「知道」你写进 SKILL.md 的工具 —— 未配置的工具即使已安装也无法被正确调用。
